...
def get_books_reccomandations():
    ...
    return jsonify(_get_recommends(book_title))

@app.route('/api/get_books_names', methods=['GET'])
def get_books_names():
    data = request.get_json()
    book_title = data.get('book_title', '')
    return jsonify({"book_names": limited_book_names}_get_right_book_name(book_title))
