...
def get_books_names():
    try:
        ...
        recommendation_books = _get_right_book_name(book_title)

        return jsonify({"books_titles": recommendations})

    except Exception as e:
        return jsonify({"error": str(e)}), 500
@app.route('/api/get_books_reccomandations', methods=['GET'])
def get_books_reccomandations(data):
    try:
        data = request.get_json()
        book_title = data.get('book_title', '')

        # Replace this with your actual logic for getting book recommendations
        recommendations = _get_right_book_name(book_title)
