...
def _get_right_book_name(book_title):
    ...
    global combine_book_rating
    pattern = book_title

    # Use str.contains() to search for the pattern in the specified column
    result = combine_book_rating['title'].str.contains(pattern, case=False)
    ...
    for index in true_indexes:
        match_books.add(combine_book_rating['title'][index])
    
    print(list(match_books)[:10])
