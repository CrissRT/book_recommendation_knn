...
function App() {
  ...
  const handleSearch = async () => {
    try {
      ...
      // console.log("console data: ", response.data.books_returned)
      const arr_authors = []
      const arr_books = []
  ...
  const handleInput = async (value) => {
    try {
      setBooks(value)
      const response = await axios.post('http://127.0.0.1:5000/api/get_books_names', { book_title_precise: bookTitle });
      // console.log("console data: ", response.data.books_returned)
      const arr_authors = []
      const arr_books = []
      for (const key in response.data.books_returned) {
