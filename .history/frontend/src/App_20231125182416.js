...
function App() {
  ...
  const handleInputRequest = asinc () => {
    try {
      const response = await axios.post('http://127.0.0.1:5000/api/get_books_names', { books_titles: bookTitle });
      const arr_books = []
      for (const key in response.data.books_titles) {
        if (response.data.books_titles.hasOwnProperty(key)) {
          arr_books.push(key);
        }
      }
